...
namespace TestMonkey.Assertion
{
    ...
    {
        private static readonly Type AsyncStateMachineAttribute =
            Type.GetType("System.Runtime.CompilerServices.AsyncStateMachineAttribute");
        ...
        private static void PreserveStackTrace(Exception ex)
        {
            if (PreserveStackTraceMethod != null)
            {
                if (preserveStackTraceDelegate == null)
                    preserveStackTraceDelegate =
                        (Action<Exception>)
                        Delegate.CreateDelegate(typeof (Action<Exception>), PreserveStackTraceMethod);
...
at wrapping a non-async method invocation in an async region was done");
        ...
        private class AsyncTaskInvocationRegion : AsyncInvocationRegion
        {
            ...
            private const BindingFlags TaskResultPropertyBindingFlags =
            ...
            public override object WaitForPendingOperationsToComplete(object invocationResult)
            {
                ...
                PropertyInfo taskResultProperty = invocationResult.GetType()
                                                                  .GetProperty(TaskResultProperty,
                                                                               TaskResultPropertyBindingFlags);

                return taskResultProperty != null
                           ? taskResultProperty.GetValue(invocationResult, null)
                           : invocationResult;
            }
            ...
            private static IList<Exception> GetAllExceptions(Exception exception)
            {
                if (SystemAggregateException.Equals(exception.GetType().FullName))
                    return
                        (IList<Exception>)
                        exception.GetType().GetProperty(InnerExceptionsProperty).GetValue(exception, null);
