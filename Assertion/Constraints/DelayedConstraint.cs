...
namespace TestMonkey.Assertion.Constraints
{
    ...
    public class DelayedConstraint : PrefixConstraint
    {
        ...
        ///     If the value of <paramref name="delayInMilliseconds" /> is less than 0
        /// </exception>
...
#endif
        {
            ...
            while (pollingInterval > 0 && pollingInterval < remainingDelay)
            {
                remainingDelay -= pollingInterval;
                Thread.Sleep(pollingInterval);
                internalActual = InvokeDelegate(del);
                try
                {
                    if (baseConstraint.Matches(internalActual))
                        return true;
                }
                catch
                {
            ...
            internalActual = InvokeDelegate(del);
            return baseConstraint.Matches(internalActual);
        }
    ...
    /// calling the base constraint with the dereferenced value.
    /// </summary>
    /// <param name="internalActual">A reference to the value to be tested</param>
    /// <returns>True for success, false for failure</returns>
		public override bool Matches(ref bool internalActual)
		{
			int remainingDelay = delayInMilliseconds;
